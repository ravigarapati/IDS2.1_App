...
import Electric2 from '../../src/pages/UnitConfiguration/Electric2';
...
describe('Electric2 screen', () => {
  beforeEach(() => {
    component = render(
      <Provider store={store}>
        <Electric2 navigation={fakeNavigation} />
      </Provider>,
    );
  });

  it('Render Electric2 screen', () => {
  ...
  it('Press option Dehumidifier and turnOn Cool to Dehumidify switch', () => {
    ...
    addSpy();
    expect(store.getState().homeOwner.infoUnitConfiguration.dehumidType).toBe(
      1,
    );
  });
  ...
  it('Press option none', () => {
    ...
    addSpy();
    expect(store.getState().homeOwner.infoUnitConfiguration.humidityConf).toBe(
      2,
    );
  });
});
